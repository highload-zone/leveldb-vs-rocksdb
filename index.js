...
const benchmark = new Benchmarkify("Pool benchmark");
...
bench.add("rocksdb put", done => {
    rdb.put(list.pop(), 0, done);
});
bench.add("leveldb put", done => {
    ldb.put(list2.pop(), 0, done);
});

let bench2 = benchmark.createSuite("RocksDB vs LevelDB batch", { time: 5000, minSamples: 3 });
bench2.add("rocksdb batch 1000", done => {
    rdb.batch(batch.pop(), done);
});
bench2.add("leveldb batch 1000", done => {
    ldb.batch(batch2.pop(), done);
});
bench2.add("rocksdb batch 10000", done => {
    rdb.batch(batch3.pop(), done);
});
bench2.add("leveldb batch 10000", done => {
    ldb.batch(batch4.pop(), done);
});
bench2.add("rocksdb batch 100000", done => {
    rdb.batch(batch5.pop(), done);
});
bench2.add("leveldb batch 100000", done => {
    ldb.batch(batch6.pop(), done);
});
...
generate.run().then(() => {
    ...
    ldb.open({
        ...
        benchmark.run([bench,bench2]).then(() => {
...
function partition(array, n) {
    return array.length ? [array.splice(0, n).map(key => ({ type: 'put', key, value: 0 }))].concat(partition(array, n)) : [];
}